...
class SessionHistory extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _SessionHistory();
}
...
class _SessionHistory extends State<SessionHistory>{
  ...
  Future<ListBooking> fetchListSession () async{
    final prefs = await SharedPreferences.getInstance();
    Token access = Token.fromJson(jsonDecode(prefs.getString('accessToken') ?? '{"token": "0", "expires":"0"}'));

    
    int timestamp = DateTime.now().millisecondsSinceEpoch - (1000 * 60 * 1);

    var res = await http.get(Uri.parse(APILINK + "booking/list/student?page=" + page.toString() + "&perPage=20&dateTimeLte=" + timestamp.toString() + "&orderBy=meeting&sortBy=desc"),
                headers: {
        ...
        child: FutureBuilder<ListBooking>(
          ...
          builder: (BuildContext context, AsyncSnapshot snapshot) {
            ...
            if (snapshot.hasData) {
              ...
              return ListView(
                children: snapshot.data.rows.map<Widget>((e) => SessionHistoryCard(e)).toList() + 
                  [Container(
